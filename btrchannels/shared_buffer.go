...
type sharedBufferChannel[T any] struct {
	in     chan T
	out    chan T
	buf    *btrstruct.Queue[T]
	closed bool
}

func (sch *sharedBufferChannel[T]) In() chan<- T {
	return sch.in
}

func (sch *sharedBufferChannel[T]) Out() <-chan T {
	return sch.out
}

func (sch *sharedBufferChannel[T]) Close() {
	close(sch.in)
}
...
type SharedBuffer[T any] struct {
	...
	chans []*sharedBufferChannel[T] // n of these
	count int
	size  BufferCap
	in    chan *sharedBufferChannel[T]
}
...
func NewSharedBuffer[T any](size BufferCap) *SharedBuffer[T] {
	...
	buf := &SharedBuffer[T]{
		size: size,
		in:   make(chan *sharedBufferChannel[T]),
	}
...
func (buf *SharedBuffer[T]) NewChannel() SimpleChannel[T] {
	ch := &sharedBufferChannel[T]{
		in:  make(chan T),
		out: make(chan T),
		buf: btrstruct.NewQueue[T](),
	}
	buf.in <- ch
	return ch
}
...
func (buf *SharedBuffer[T]) mainLoop() {
	for {
		...
		if i == 0 {
			...
			ch := val.Interface().(*sharedBufferChannel[T])
			...
			if ok {
				...
				if ch.buf.Length() == 0 && !buf.cases[i+1].Chan.IsValid() {
					...
					ch.buf.Add(val.Interface().(T)) // FIXME
				}
				if buf.count == int(buf.size) {
